...
class ArticleForm(RenderForm):
    article_url = StringField(label='',
                              render_kw={"placeholder": "0-20个字符", "id": "tx_input_2", "class": "invisible", "type": "text",
                                         "readonly": "readonly"})
    article_title = StringField(label='标题', validators=[Length(min=5, max=255, message="必须介于5-255个字符")],
                                render_kw={"placeholder": "5-255个字符"})
    article_summary = TextAreaField(label='正文', validators=[Length(min=15, max=10000, message="必须介于15-3000个字符")],
                                    render_kw={"placeholder": "15-10000个字符", "id": "editor"})
...
class CommentForm(RenderForm):
    comment_text = StringField(label='标题', validators=[Length(min=5, max=255, message="必须介于5-255个字符")],
                               render_kw={"placeholder": "5-255个字符"})
